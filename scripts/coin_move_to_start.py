...
ros.init_node('coin_move_to_start')

action = ros.resolve_name('~follow_joint_trajectory')
client = SimpleActionClient(action, FollowJointTrajectoryAction)
ros.loginfo("coin_move_to_start: Waiting for '" + action + "' action to come up")
client.wait_for_server()

param = ros.resolve_name('~joint_pose')
pose = ros.get_param(param, None)
if pose is None:
    ros.logerr('coin_move_to_start: Could not find required parameter "' + param + '"')
    sys.exit(1)

topic = ros.resolve_name('~joint_states')
ros.loginfo("coin_move_to_start: Waiting for message on topic '" + topic + "'")
...
if result.error_code != FollowJointTrajectoryResult.SUCCESSFUL:
    ros.logerr('coin_move_to_start: Movement was not successful: ' + {
    ...
    ros.loginfo('coin_move_to_start: Successfully moved into start pose')
